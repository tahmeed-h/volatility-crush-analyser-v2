...
class VolatilityCrushAnalyzer:
    ...
    def setup_connection_section(self, parent, row):
        ...
        ttk.Entry(conn_frame, textvariable=self.host_var, width=15).grid(row=0, column=1, padx=(0, 15),
                                                                         sticky=(tk.W, tk.E))

        ttk.Label(conn_frame, text="Port:").grid(row=0, column=2, padx=(0, 5), sticky=(tk.W, tk.E))
        self.port_var = tk.StringVar(value='7497')
        ttk.Entry(conn_frame, textvariable=self.port_var, width=10).grid(row=0, column=3, padx=(0, 15),
                                                                         sticky=(tk.W, tk.E))
    ...
    def setup_market_data_section(self, parent, row):
        ...
        ttk.Entry(data_frame, textvariable=self.spot_price_var, width=15, font=('Arial', 10, 'bold')).grid(row=1,
                                                                                                           column=1,
                                                                                                           sticky=(tk.W,
                                                                                                                   tk.E),
                                                                                                           pady=(0, 8))
        ...
        ttk.Entry(data_frame, textvariable=self.strike_var, width=15, font=('Arial', 10, 'bold')).grid(row=2, column=1,
                                                                                                       sticky=(tk.W,
                                                                                                               tk.E),
                                                                                                       pady=(0, 8))
        ...
        ttk.Entry(data_frame, textvariable=self.iv_var, width=15, font=('Arial', 10, 'bold')).grid(row=3, column=1,
                                                                                                   sticky=(tk.W, tk.E),
                                                                                                   pady=(0, 8))

        ttk.Label(data_frame, text='Days to Expiry:').grid(row=4, column=0, padx=(0, 10), pady=(0, 8), sticky=tk.W)
        self.days_var = tk.StringVar(value="30")
        ttk.Entry(data_frame, textvariable=self.days_var, width=15, font=('Arial', 10, 'bold')).grid(row=4, column=1,
                                                                                                     sticky=(tk.W,
                                                                                                             tk.E),
                                                                                                     pady=(0, 8))
    ...
    def setup_scenario_section(self, parent, row):
        ...
        ttk.Entry(scenario_frame, textvariable=self.new_spot_price, width=15, font=('Arial', 10, 'bold')).grid(
            row=0, column=1, sticky=(tk.W, tk.E), pady=(0, 8)
        )

        ttk.Label(scenario_frame, text='New IV (%):').grid(row=1, column=0, padx=(0, 10), pady=(0, 8), sticky=tk.W)
        self.new_iv_var = tk.StringVar()
        ttk.Entry(scenario_frame, textvariable=self.new_iv_var, width=15, font=('Arial', 10, 'bold')).grid(row=1, column=1,
                                                                                                           sticky=(tk.W,
                                                                                                                   tk.E),
                                                                                                           pady=(0, 8))

        self.analyze_btn = ttk.Button(scenario_frame, text="Analyze Scenario", command=self.analyze_scenario,
                                      state="disabled")
    ...
    def clear_data(self):
        ...
        for label in labels_to_reset:
            # simple reset
            label.config(text="$0.00", foreground='black') if '$' in str(label) else label.config(text='0.00',
                                                                                                   foreground='black')
    ...
    def price_current_straddle(self):
        ...
        delta = self.calculate_delta(spot_price, strike_price, T, r, iv_decimal, 'call') + self.calculate_delta(
            spot_price, strike_price, T, r, iv_decimal, 'put')
        ...
        theta = self.calculate_theta(spot_price, strike_price, T, r, iv_decimal, 'call') + self.calculate_theta(
            spot_price, strike_price, T, r, iv_decimal, 'put')
    ...
    def analyze_scenario(self):
        ...
        new_delta = self.calculate_delta(new_spot, K, T, r, new_iv, 'call') + self.calculate_delta(new_spot, K, T, r,
                                                                                                   new_iv, 'put')
        new_gamma = self.calculate_gamma(new_spot, K, T, r, new_iv)
        new_vega = self.calculate_vega(new_spot, K, T, r, new_iv) * 2
        new_theta = self.calculate_theta(new_spot, K, T, r, new_iv, 'call') + self.calculate_theta(new_spot, K, T, r,
                                                                                                   new_iv, 'put')
